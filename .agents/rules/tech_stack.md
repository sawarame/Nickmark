...
- **TypeScript**: 静的型付け言語。全てのスクリプト（バックグラウンド、オプション画面）で使用。
- **Vue.js (Vue 3)**: オプション画面の UI 構築のための Composition API ベースのフレームワーク。
...
- `src/`: 開発用のソースコードディレクトリ。
  ...
  - `options.ts`: オプション画面のエントリーポイント。
  - `Options.vue`: オプション画面のメインコンポーネント。
- `Nickmark/`: 拡張機能としてブラウザに読み込ませる（配布する）ディレクトリ。
  - `manifest.json`: 拡張機能の定義ファイル。
  - `options.html`: オプション画面のベース HTML。
